#include "Texte.h"

using namespace std ;

#define TYPEFACE 30
#define SHIFT_X 25

Texte::Texte(const char * textToWrite, SDL_Color font_color, int size){
	/* Loading of the font */
  m_size = size ;
	m_font = TTF_OpenFont ("../font/04B_30__.TTF", size) ;
	if(!m_font) {
    	cout << TTF_GetError() << endl ;
    	exit(3) ;
	}

	/* Writting the text in the SDL_Surface */
	m_text = TTF_RenderText_Blended(m_font,textToWrite,font_color);
}

Texte::Texte(string textToWrite, SDL_Color font_color, int size) : Texte(textToWrite.c_str(), font_color, size)
{}

Texte::~Texte(){
	TTF_CloseFont(m_font) ;
}

/***************************************************************************************/

int Texte::width(){
	return m_text->w ;
}

int Texte::height(){
	return m_text->h ;
}

SurfaceAffichage Texte::surfaceAffichage(){

	SurfaceAffichage surface(this->width(), this->height(), this->m_text) ;
	return surface ;
}

void Texte::displayText(Fenetre& screen, AbstractButton * b){

	int pos_x = b->getPosX() + ( b->getWidth() - m_text->w )/2  ;
	int pos_y = b->getPosY() + ( b->getHeight() - m_text->h) /2 ;

	SurfaceAffichage surface = this->surfaceAffichage() ;

	screen.ajouter(surface, pos_x, pos_y) ;
}